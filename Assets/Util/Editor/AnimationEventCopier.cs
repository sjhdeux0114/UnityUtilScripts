...
public class AnimationEventCopier : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (sourceObject != null && targetObject != null)
        {
            ...
            if (GUILayout.Button("Add Single Clip"))
            {
                Undo.RecordObject(targetObject, "Undo Clip Event add");
                CopyData_add(sourceObject, targetObject);
    ...
    void CopyData_add(AnimationClip sourceClip, AnimationClip targetClip)
    {
        //AnimationUtility.SetAnimationEvents(targetClip, null);                                                                // Remove all events
        AnimationEvent[] aniEvent_T = AnimationUtility.GetAnimationEvents(targetClip);
        AnimationEvent[] aniEvent_S = AnimationUtility.GetAnimationEvents(sourceClip);

        AnimationEvent[] ret_Event = new AnimationEvent[aniEvent_T.Length + aniEvent_S.Length];
        int p = 0;
        for (int i = 0; i < aniEvent_T.Length; i++)
        {
            ret_Event[p] = aniEvent_T[i];
            p++;
        }
        for (int i = 0; i < aniEvent_S.Length; i++)
        {
            ret_Event[p] = aniEvent_S[i];
            p++;

        }
        AnimationUtility.SetAnimationEvents(targetClip, ret_Event);                    // Copy Events
        //AnimationUtility.SetAnimationClipSettings(targetClip, AnimationUtility.GetAnimationClipSettings(sourceClip));        // Copy Clip Settings
        Debug.Log("Copying " + AnimationUtility.GetAnimationEvents(sourceClip).Length + " event from " + sourceClip.name + " to " + targetClip.name + " (" + AnimationUtility.GetAnimationEvents(targetClip).Length + " events)");
    }
